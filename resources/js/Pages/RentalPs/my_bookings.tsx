...
export default function MyBookings({ bookings }: Props) {
    ...
    return (
        <div className="min-h-screen bg-background flex flex-col">
            <Head title="Booking Saya – RentalPs" />
            <Navbar />
            ...
            <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-grow w-full">
                ...
                <div className="mb-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-body-l text-foreground/60 mt-1">Pantau semua riwayat booking rental-ps Anda.</p>
                    </div>
                    <Link
                        href="/rental-ps"
                ...
                {bookings.length === 0 ? (
                    <div className="bg-card border border-border rounded-venus p-12 text-center">
                        ...
                        <p className="text-body-reg text-foreground/50 mt-2">Mulai booking layanan rental-ps pertama Anda.</p>
                        <Link href="/rental-ps" className="mt-5 inline-block bg-secondary text-secondary-foreground px-6 py-3 rounded-full text-label-sm font-bold hover:bg-secondary/90 transition-all">
                            Booking Sekarang
                        </Link>
                    </div>
                ) : (
...
function BookingCard({ booking }: { booking: Booking }) {
    ...
    return (
        <Link
            href={`/rental-ps/tracking/${booking.booking_code}`}
