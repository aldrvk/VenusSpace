...
export default function BookingDetail() {
    ...
    const handleConfirm = () => {
        ...
        localStorage.setItem('rentalps_booking', JSON.stringify(confirmed));
        setTimeout(() => {
            router.visit('/rental-ps/booking-receipt');
        }, 600);
    };

    return (
        <div className="min-h-screen bg-background flex flex-col">
            <Head title="Detail Booking – rental-ps" />
            <Navbar />
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-grow w-full">
                ...
                <div className="flex items-center gap-2 mb-6 text-label-sm text-foreground/50 uppercase">
                    <Link href="/rental-ps" className="hover:text-primary transition-colors flex items-center gap-1.5">
                        <ArrowLeftIcon /> rental-ps
                    </Link>
                    <span>›</span>
