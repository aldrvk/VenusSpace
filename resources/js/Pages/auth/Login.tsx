    ...
    return (
        <>
            ...
            <div className={`fixed inset-0 z-[100] flex items-center justify-center overflow-hidden bg-black/20 backdrop-blur-3xl font-sans p-3 sm:p-6 lg:p-8 transition-opacity duration-500 ${isLoaded ? 'opacity-100' : 'opacity-0'}`} onClick={onClose}>
                ...
                <div className={`absolute top-0 left-0 w-[300px] sm:w-[500px] h-[300px] sm:h-[500px] bg-[#40dfc0] rounded-full mix-blend-multiply filter blur-[80px] sm:blur-[100px] opacity-40 transition-transform duration-[2000ms] ${isLoaded ? 'translate-x-0 translate-y-0' : '-translate-x-20 -translate-y-20'}`} />
                <div className={`absolute bottom-0 right-0 w-[350px] sm:w-[600px] h-[350px] sm:h-[600px] bg-[#0b5c5a] rounded-full mix-blend-multiply filter blur-[90px] sm:blur-[120px] opacity-30 transition-transform duration-[2000ms] ${isLoaded ? 'translate-x-0 translate-y-0' : 'translate-x-20 translate-y-20'}`} />

                {/* Card Modal */}
                <div
                    onClick={(e) => e.stopPropagation()}
                    className={`relative flex flex-col w-full max-w-[480px] min-h-[auto] rounded-2xl sm:rounded-[2.5rem] overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.1)] lg:shadow-[0_30px_60px_rgba(0,0,0,0.12)] border border-white/60 backdrop-blur-3xl transition-all duration-1000 ease-out transform ${
                ...
                >
                    ...
                    {/* ── Form Bawah ── */}
                    <div className="w-full flex items-center justify-center p-6 sm:p-10 bg-white/40">
                        <div className="w-full">
                            <h2 className={`text-[#065F51] text-h3 font-heading mb-8 transition-all duration-700 delay-300 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                                Masuk ke akun Anda
                            </h2>
                            ...
                            <form onSubmit={handleLogin} className="space-y-5">
                                ...
                                <div className={`transition-all duration-700 delay-700 ${isLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
                                    <div className="flex justify-end mb-1">
                                        {onSwitch ? (
                                            <button type="button" onClick={() => onSwitch('forgot-password')} className="text-[#3cdbc0] text-labelSm font-sans uppercase hover:text-[#2ba898] transition-colors">
                                                LUPA KATA SANDI?
                                            </button>
                                        ) : (
                                            <Link href="/forgot-password" className="text-[#3cdbc0] text-labelSm font-sans uppercase hover:text-[#2ba898] transition-colors">
                                                LUPA KATA SANDI?
                                            </Link>
                                        )}
                                    </div>
                                ...
                                <div className={`flex items-center transition-all duration-700 delay-[800ms] ${isLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
                                    <input
                                        ...
                                        className="w-4 h-4 text-[#3cdbc0] bg-white border-gray-300 rounded focus:ring-[#3cdbc0] focus:ring-2"
                                    />
                                    <label htmlFor="remember" className="ml-2 text-bodyM font-sans text-gray-600 cursor-pointer">
                                        Ingat Saya
                                    </label>
                                </div>
                            ...
                            <p className={`mt-8 text-center text-body font-sans text-[#4b5563] transition-all duration-700 delay-[1200ms] ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
                                Belum punya akun?{' '}
                                {onSwitch ? (
                                            <button
                                                type="button"
