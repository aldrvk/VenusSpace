...
const strengthConfig = {
    ...
    weak:          { label: 'Lemah',        bars: 1, color: 'bg-red-500',    text: 'text-red-500' },
    strong:        { label: 'Kuat',         bars: 2, color: 'bg-yellow-400', text: 'text-yellow-500' },
    'very-strong': { label: 'Sangat Kuat',  bars: 3, color: 'bg-[#3cdbc0]',  text: 'text-[#2ba898]' },
};
...
export default function Register({ isOpen = true, onClose, onSwitch }: AuthModalProps) {
    ...
    return (
        <>
            ...
            {/* Modal Overlay */}
            <div className={`fixed inset-0 z-[100] flex items-center justify-center overflow-hidden bg-black/20 backdrop-blur-3xl font-sans p-3 sm:p-6 lg:p-8 transition-opacity duration-500 ${isLoaded ? 'opacity-100' : 'opacity-0'}`} onClick={onClose}>

                {/* Dekorasi blob latar belakang */}
                <div className={`absolute top-0 right-0 w-[300px] sm:w-[500px] h-[300px] sm:h-[500px] bg-[#40dfc0] rounded-full mix-blend-multiply filter blur-[80px] sm:blur-[100px] opacity-40 transition-transform duration-[2000ms] ${isLoaded ? 'translate-x-0 translate-y-0' : 'translate-x-20 -translate-y-20'}`} />
                <div className={`absolute bottom-0 left-0 w-[350px] sm:w-[600px] h-[350px] sm:h-[600px] bg-[#0b5c5a] rounded-full mix-blend-multiply filter blur-[90px] sm:blur-[120px] opacity-30 transition-transform duration-[2000ms] ${isLoaded ? 'translate-x-0 translate-y-0' : '-translate-x-20 translate-y-20'}`} />
                ...
                <div
                    ...
                    style={{ background: 'linear-gradient(135deg,rgba(255,255,255,.85) 0%,rgba(255,255,255,.4) 100%)' }}
                >
                    {/* ── Banner Atas ── */}
                    ...
                    {/* ── Form Bawah ── */}
                    <div className="w-full flex flex-col bg-white/40 max-h-[60vh] sm:max-h-[70vh] overflow-y-auto">
                        <div className="flex-1 flex items-start justify-center p-6 sm:p-10">
                            <div className="w-full py-2">
                            <h2 className={`text-[#065F51] text-h3 font-heading mb-6 transition-all duration-700 delay-300 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                                Buat Akun Baru
                            </h2>
                            ...
                            <form onSubmit={handleRegister} className="space-y-4">
                                ...
                                <div className={`transition-all duration-700 delay-[350ms] ${isLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
                                    ...
                                    />
                                    {errors.name && <p className="text-red-500 text-body font-sans mt-1 ml-1">{errors.name}</p>}
                                </div>
                                ...
                                <div className={`transition-all duration-700 delay-[450ms] ${isLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
                                    ...
                                    />
                                    {errors.email && <p className="text-red-500 text-body font-sans mt-1 ml-1">{errors.email}</p>}
                                </div>
                                ...
                                <div className={`transition-all duration-700 delay-[550ms] ${isLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
                                    ...
                                    />
                                    {errors.password && <p className="text-red-500 text-body font-sans mt-1 ml-1">{errors.password}</p>}
                                    ...
                                    {data.password.length > 0 && (
                                        <div className="mt-2 px-1">
                                            ...
                                            <p className={`text-labelSm font-sans ${cfg.text}`}>{cfg.label}</p>
                                        </div>
                                    )}
                                    ...
                                    {data.password.length > 0 && (
                                        <ul className="mt-2 px-1 space-y-0.5">
                                            {[
                                                ...
                                                return (
                                                    <li key={key} className={`flex items-center gap-1.5 text-body font-sans transition-colors ${ok ? 'text-[#2ba898]' : 'text-gray-400'}`}>
                                ...
                                <div className={`transition-all duration-700 delay-[650ms] ${isLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
                                    ...
                                    {data.password_confirmation.length > 0 && (
                                        <p className={`text-labelSm font-sans mt-1 ml-1 transition-colors ${data.password === data.password_confirmation ? 'text-[#2ba898]' : 'text-red-500'}`}>
                                ...
                                <div className={`flex items-start pt-1 transition-all duration-700 delay-[750ms] ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
                                    <input
                                        ...
                                        className="mt-0.5 w-4 h-4 rounded text-[#3cdbc0] border-gray-300 focus:ring-[#3cdbc0] cursor-pointer"
                                    />
                                    <label htmlFor="terms" className="ml-2 text-bodyM font-sans text-gray-600 cursor-pointer">
                                        Saya menyetujui{' '}
                                        <a href="#" className="text-[#3cdbc0] hover:underline font-semibold">Syarat & Ketentuan</a>
                                        {' '}dan{' '}
                                        <a href="#" className="text-[#3cdbc0] hover:underline font-semibold">Kebijakan Privasi</a>
                                    </label>
                                </div>
                                ...
                                <button
                                    type="submit"
                                    disabled={processing}
                                    className={`w-full bg-gradient-to-r from-[#3cdbc0] to-[#2ba898] hover:to-[#218c7e] text-white text-labelSm font-sans uppercase py-4 rounded-xl transition-all duration-300 transform hover:-translate-y-1 hover:shadow-[0_10px_20px_-5px_rgba(60,219,192,0.5)] shadow-[0_4px_10px_-2px_rgba(60,219,192,0.3)] disabled:opacity-60 disabled:cursor-not-allowed ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                                >
                                    {processing ? 'Memproses...' : 'Daftar Sekarang'}
                                </button>
                            </form>

                            <p className={`mt-6 text-center text-body font-sans text-[#4b5563] transition-all duration-700 delay-[950ms] ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
                                Sudah punya akun?{' '}
                                {onSwitch ? (
                                    <button
                                        type="button"
                                    ...
                                    <Link href="/login" className="text-[#2ba898] font-bold hover:text-[#1c786c] transition-colors relative after:absolute after:bottom-0 after:left-0 after:w-full after:h-[2px] after:bg-[#2ba898] after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:origin-left">
                                        Masuk di sini
                                    </Link>
                                )}
                            </p>
