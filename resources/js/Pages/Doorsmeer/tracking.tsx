...
const VENUS_ADS: PromoAd[] = [
    {
        ...
        description: 'Sambil nunggu mobil dicuci, cek koleksi pod system & liquid terbaru di Venus Vape Store.',
    ...
    },
    {
        ...
        description: 'Nikmati segelas espresso artisan sambil menunggu kendaraan Anda selesai dicuci.',
    ...
    },
    {
        ...
        description: 'Ajak teman main PS5 sambil nunggu mobil dicuci. Tersedia ruangan privat ber-AC.',
...
const STEPS = [
    ...
    },
    {
        key: 'in_queue',
        label: 'Dalam Antrian',
        desc: 'Menunggu bay pencucian tersedia.',
        icon: (
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    },
    {
        key: 'washing',
        label: 'Sedang Dicuci',
        desc: 'Kendaraan sedang dicuci oleh tim kami.',
        icon: (
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function TrackingPage({ booking: initialBooking, showAd: shouldShowAd }: Props) {
    ...
    const handleCancelBooking = () => {
        if (window.confirm("Apakah Anda yakin ingin membatalkan booking ini?")) {
            router.post(`/doorsmeer/cancel/${booking.id}`, {}, {
                onSuccess: () => {
    ...
    const STATUS_TOAST: Partial<Record<string, { msg: string; type: 'success' | 'error' | 'loading' | 'default' }>> = {
        verified:  { msg: 'Kedatangan Anda dikonfirmasi! Menunggu bay...', type: 'success' },
            ...
            <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-grow w-full">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-4">
                        ...
                        {booking.status === 'cancelled' ? (
                            <div className="bg-red-50 border border-red-200 rounded-venus p-8 text-center flex flex-col items-center">
                                ...
                                <Link href="/doorsmeer" className="mt-8 bg-red-600 text-white px-6 py-3 rounded-full text-label-sm font-bold hover:bg-red-700 transition-all shadow-lg">
                                    Booking Ulang
                                </Link>
                            </div>
                        ) : (
                            <>
                                ...
                                {booking.stall && (
                                    <div className="bg-primary/5 border border-primary/20 rounded-venus p-4 flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-label-sm text-foreground/50 uppercase">Bay Pencucian</p>
                                ...
                                {booking.status === 'in_queue' && (
                                    <div className="bg-purple-50 border border-purple-200 rounded-venus p-4 flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-h4 text-purple-700">Menunggu bay tersedia</p>
                                            <p className="text-body-reg text-foreground/50 mt-0.5">Kendaraan Anda akan otomatis dipindahkan saat bay kosong.</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                {booking.status === 'done' && (
                                    <div className="bg-emerald-50 border border-emerald-200 rounded-venus p-5 flex gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-body-reg text-emerald-600 mt-1">Proses pencucian telah selesai. Silakan ambil kendaraan Anda di area doorsmeer.</p>
                                        </div>
                                    </div>
                                )}
                            </>
                    ...
                    <div className="space-y-4">
                        ...
                        <Link href="/doorsmeer/my-bookings" className="block text-center text-label-sm text-primary hover:text-primary/80 font-semibold transition-colors py-2">
