...
export default function BookingDetail() {
    ...
    useEffect(() => {
        const raw = localStorage.getItem('doorsmeer_booking');
        if (raw) {
            setBooking(JSON.parse(raw));
        } else {
            router.visit('/doorsmeer');
        }
    }, []);
    ...
    const handleConfirm = () => {
        ...
        localStorage.setItem('doorsmeer_booking', JSON.stringify(confirmed));
        setTimeout(() => {
            router.visit('/doorsmeer/booking-receipt');
        }, 600);
    };

    return (
        <div className="min-h-screen bg-background flex flex-col">
            <Head title="Detail Booking – Doorsmeer" />
            <Navbar />
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-grow w-full">
                ...
                <div className="flex items-center gap-2 mb-6 text-label-sm text-foreground/50 uppercase">
                    <Link href="/doorsmeer" className="hover:text-primary transition-colors flex items-center gap-1.5">
                        <ArrowLeftIcon /> Doorsmeer
                    </Link>
                    <span>›</span>
