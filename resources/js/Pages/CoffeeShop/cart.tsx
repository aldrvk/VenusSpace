...
export default function Cart() {
    ...
    return (
        <div className="min-h-screen bg-background flex flex-col">
            <Head title="Coffee Shop - Cart" />
            <Navbar />
            <main className="flex-grow max-w-7xl mx-auto px-6 py-12 w-full">
                <div className="mb-16">
                    <h1 className="text-h1 text-super-black mb-4">Your Items</h1>
                    <p className="text-body-l text-foreground/80 max-w-2xl">Review your cart. Each item is crafted with care and the finest ingredients.</p>
                </div>
                {cartItems.length === 0 ? (
                    <div className="text-center py-24 bg-card rounded-venus border border-border">
                        ...
                        <p className="text-body-m text-foreground/60 mb-8">Discover our premium selection of coffee and snacks.</p>
                        <Link href="/coffee-shop" className="inline-flex items-center gap-2 bg-secondary text-secondary-foreground px-8 py-4 rounded-full text-label-sm transition-all hover:bg-secondary/90 shadow-lg">CONTINUE SHOPPING</Link>
                    </div>
                ) : (
                    <div className="flex flex-col lg:flex-row gap-12">
                                    ...
                                    <div key={item.cartItemId} className="bg-card rounded-venus p-4 md:p-6 flex flex-col md:flex-row items-center gap-6 border border-border shadow-sm">
                                        <div className="flex-[2] flex items-center gap-6 w-full">
                                            ...
                                            <div className="flex flex-col gap-2">
                                                ...
                                                <button onClick={() => removeItem(item.cartItemId)} className="text-label-sm text-error flex items-center gap-1 hover:underline w-fit mt-1 uppercase tracking-widest">
                                        ...
                                        <div className="flex-1 flex justify-center w-full md:w-auto mt-4 md:mt-0">
                                            <div className="flex items-center bg-surface rounded-full h-12 border border-border w-32">
                                                <button onClick={() => updateQuantity(item.cartItemId, item.quantity - 1)} className="w-10 h-full flex items-center justify-center text-super-black hover:text-primary transition-colors text-h4">-</button>
                                                <span className="flex-1 text-center text-body-m font-bold">{item.quantity < 10 ? `0${item.quantity}` : item.quantity}</span>
                                                <button onClick={() => updateQuantity(item.cartItemId, item.quantity + 1)} className="w-10 h-full flex items-center justify-center text-super-black hover:text-primary transition-colors text-h4">+</button>
                                            </div>
                                        </div>
                        ...
                        <div className="w-full lg:w-[400px] flex-shrink-0">
                            <div className="bg-surface rounded-venus p-8 sticky top-24 border border-border">
                                ...
                                <button className="w-full bg-primary text-primary-foreground py-4 rounded-venus text-label-sm tracking-widest text-center hover:bg-primary/90 transition-all font-bold shadow-lg mt-8">PROCEED TO CHECKOUT</button>
