...
export default function TrackingPage({ booking: initialBooking }: Props) {
    ...
    const handleCancelBooking = () => {
        if (window.confirm("Apakah Anda yakin ingin membatalkan booking ini?")) {
            router.post(`/bengkel/cancel/${booking.id}`, {}, {
                onSuccess: () => {
    ...
    useEffect(() => {
        ...
        const poll = async () => {
            try {
                const res = await fetch(`/api/bengkel/status/${booking.booking_code}`, {
            ...
            <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-grow w-full">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                ...
                                {booking.status === 'in_queue' && (
                                    <div className="bg-purple-50 border border-purple-200 rounded-venus p-4 flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-h4 text-purple-700">Menunggu pit tersedia</p>
                                            <p className="text-body-reg text-foreground/50 mt-0.5">Kendaraan Anda akan otomatis dipindahkan saat pit kosong.</p>
                                        </div>
                                    </div>
                                )}
                    ...
                    <div className="space-y-4">
                        ...
                        <Link href="/bengkel/my-bookings" className="block text-center text-label-sm text-primary hover:text-primary/80 font-semibold transition-colors py-2">
